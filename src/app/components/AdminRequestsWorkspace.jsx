...
export function AdminRequestsWorkspace({ data, companyId, profile, reload, onOpenWorkOrders }) {
  ...
  function openRequest(request) {
    const requestFacility = data.facilities.find(item => item.id === request.facility_id);
    ...
    setSchedule({
      ...
      estimated_minutes: requestFacility?.estimated_minutes || 90,
      area_names: '',
  ...
  if (selected) return <div className="page arwPage">
    ...
    {linkedOrder ? <section className="panel arwLinked"><div><ClipboardCheck size={22}/><div><p className="eyebrow">Linked work order</p><h2>{linkedOrder.title}</h2><span>{dateLabel(linkedOrder.scheduled_date)} · {linkedOrder.scheduled_time || 'Any time'}</span></div></div><button className="btn secondary" onClick={onOpenWorkOrders}>Open work orders <ChevronRight size={16}/></button></section> : <section className="panel arwScheduler">
  ...
  const counts = { new:data.requests.filter(item => item.status === 'new').length, reviewing:data.requests.filter(item => item.status === 'reviewing').length, scheduled:data.requests.filter(item => item.status === 'scheduled').length };
  return <div className="page arwPage">
    ...
    <section className="panel arwList">{requests.map(request => { const itemCustomer = data.customers.find(item => item.id === request.customer_id); const itemFacility = data.facilities.find(item => item.id === request.facility_id); return <button className="arwRow" key={request.id} onClick={() => openRequest(request)}><div className="arwIcon"><Wrench size={19}/></div><div><strong>{request.title}</strong><span>{itemCustomer?.name || 'Customer'} · {itemFacility?.name || 'No facility'} · {String(request.request_type || '').replaceAll('_',' ')}</span></div><div className={`status ${request.status}`}>{request.status}</div><ChevronRight size={18}/></button>; })}{!requests.length && <div className="arwEmpty"><Wrench size={28}/><strong>No matching requests</strong><span>Customer submissions will appear here.</span></div>}</section>
  </div>;
}
